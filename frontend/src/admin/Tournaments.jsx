...
export default function Tournaments({ onBack }) {
  ...
  async function createTournament() {
    ...
    const { data, error } = await supabase
      .from('tournaments')
      .insert({ name: name.trim(), description: description.trim() || null, sport: sport.trim() || 'pickleball', venue: venue.trim() || null })
      .select('id')
      .single()
    setSaving(false)
    if (!error && data) {
      setName(''); setDescription(''); setSport('pickleball'); setVenue(''); setCreating(false)
      setSelected(data.id)
    }
  }
            ...
            {creating ? (
              <div className="card-compact px-4 py-3 space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setCreating(false); setName(''); setDescription(''); setSport('pickleball'); setVenue('') }} className="text-xs font-medium text-muted px-4 py-2 rounded-full border border-border active:bg-bg transition">Cancel</button>
                </div>
              </div>
            ) : (
