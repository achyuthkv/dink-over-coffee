...
export default function SessionForm({ session, onSave, onCancel }) {
  ...
  const initialTime = parseTime(session?.time)
  ...
  async function handleSubmit(e) {
    ...
    const time = form.timeFrom && form.timeTo ? `${form.timeFrom} - ${form.timeTo}` : form.timeFrom || form.timeTo || ''
    const { timeFrom, timeTo, ...rest } = form
    const payload = {
      ...rest,
      time,
        ...
        <form onSubmit={handleSubmit} className="card space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="time" className="input mt-1" value={form.timeFrom} onChange={e => update('timeFrom', e.target.value)} required />
            </div>
            <div>
              <label className="text-xs font-semibold text-primary">To</label>
              <input type="time" className="input mt-1" value={form.timeTo} onChange={e => update('timeTo', e.target.value)} required />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="text-xs font-semibold text-primary">Slots</label>
          ...
          <div>
            ...
            {upiAccounts.length === 0 ? (
              <p className="text-xs text-muted mt-1">No UPI accounts. Add them from Payment Methods.</p>
            ) : (
              <div className="mt-2 space-y-2">
                {upiAccounts.map(acc => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border transition text-left ${selectedUpis.includes(acc.id) ? 'border-secondary bg-secondary/5' : 'border-border-muted'}`}
                  >
                    <span className={`shrink-0 w-5 h-5 rounded-md border-2 flex items-center justify-center transition ${selectedUpis.includes(acc.id) ? 'bg-secondary border-secondary' : 'border-border-muted'}`}>
                    ...
                    <div className="min-w-0">
                      <span className="text-sm font-medium text-primary block truncate">{acc.label}</span>
          ...
          <div className="flex items-center gap-3 pt-1">
            <button
              ...
              className={`relative w-10 h-6 rounded-full transition-colors ${form.active ? 'bg-secondary-dark' : 'bg-border-muted'}`}
            >
              <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${form.active ? 'translate-x-4' : ''}`} />
            </button>
            <span className="text-sm font-medium text-primary">{form.active ? 'Active' : 'Inactive'}</span>
          </div>
