...
import { createCourtScene, detectTier, beatOpacity } from '../lib/courtScene.js'
...
// Cinematic WebGL hero: a scroll-scrubbed camera flythrough of a pickleball
// court (canvas pinned via position:sticky while its tall wrapper scrolls
// underneath it), with the usual hero copy staged as fading overlay beats
// instead of all appearing at once. Falls back to a plain gradient if WebGL
// genuinely isn't available -- that's a hard compatibility fallback, not a
// performance one; every capable device gets the full 3D scene per-tier.
export default function CourtFlythroughHero() {
  ...
  useEffect(() => {
    ...
    let scene
    try {
      scene = createCourtScene(canvas, { tier: detectTier() })
    } catch {
      setUnsupported(true)
      return
    }
    ...
    function update() {
      ...
      scene.setProgress(reduceMotion ? 0.35 : progress)
      applyBeats(reduceMotion ? 0.35 : progress)
    }
    update()
...
// Rides along with the sticky canvas for the whole flythrough, then
// scrolls away with it once the track ends -- matches the original hero's
// nav, which was never a persistent site-wide header either.
function NavBar() {
  return (
