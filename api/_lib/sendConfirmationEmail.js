...
<h1 style="margin:0;color:#ffffff;font-size:20px;font-weight:700;">You're in! ${title} - ${dateFormatted}</h1>
</td></tr>
</table>
...
Hey ${playerName},<br /><br />
Your spot is confirmed! We've attached a calendar invite for your convenience.<br /><br />
...
export async function sendConfirmationEmail(session, player) {
  if (!RESEND_API_KEY || !player.email) return;

  try {
    const dateFormatted = formatSessionDate(session.date);
    const subject = `You're in! ${session.title || 'Dink Over Coffee'} - ${dateFormatted} | ${session.time || ''}`;
    const html = buildConfirmationHtml(session, player.name);
