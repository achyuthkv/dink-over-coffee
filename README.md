...
api/                Vercel serverless functions — one file per route (Vercel's Hobby plan caps the function count, so routes are consolidated with an `action` field where it makes sense, e.g. `waiver.js`, `shop.js`, `tournament.js`)
  _lib/             Shared server helpers (supabase client, slot counting, atomic register, Razorpay, rate limiting, email, tournament capacity/validation, tournament session-sync) — underscore prefix excludes these from Vercel's function count
...
- **tournaments** — `id, name, description, sport, venue, start_date, end_date, status (setup|active|completed), created_at`. Just the event shell — everything format/registration/fixture-related lives one level down, on its categories. `setup` is hidden from the public `/tournament` page (organizers can stage categories before anything's visible); `active` and `completed` are public. `/tournament` shows whichever `active` tournament is newest, falling back to the newest `completed` one so results linger after an event ends.
- **tournament_categories** — `id, tournament_id, name, format (round_robin|single_elim|group_knockout), team_size (1|2), max_teams (nullable), entry_fee, early_bird_fee (nullable), early_bird_deadline (nullable date), advance_per_group, session_id (nullable, references sessions), status (setup|registration_open|registration_closed|active|completed), sort_order, created_at`. A tournament runs one or more of these in parallel (Men's Doubles, Mixed, a skill bracket, …), each with its own bracket type, entry fee and roster. `session_id` links a category back to the `sessions` row its registrations came from (e.g. a DUPR-teams event people signed up for via `/events`) and drives the auto-sync described below; unrelated to the free-standing public registration flow every category also supports directly.
...
- **tournament_teams** — `id, category_id, group_id (nullable), name, player1_name, player2_name, seed (nullable), status (confirmed|waitlisted|withdrawn), source_player_id (nullable, references players), created_at`. Public-readable columns only, by design — see `tournament_registrations` for why contact details live in a separate table. `source_player_id` marks a team as auto-synced from a session registration (see below); teams registered directly or added by hand in `/admin` leave it null.
- **tournament_registrations** — `id, team_id (unique, references tournament_teams), phone, player2_phone (nullable), email (nullable), amount, payment_status (free|pending|paid|refunded), razorpay_order_id, razorpay_payment_id, created_at`. Contact + payment details for a registration, split out of `tournament_teams` and RLS-locked to `authenticated` only (no anon policy at all, same reasoning as `upi_accounts`) — the public live-bracket/standings page selects every column of `tournament_teams` freely, so nothing sensitive can live there.
...
The full schema (tables, indexes, RLS policies, and the session auto-sync trigger) is in `supabase/migrations/0001_tournament_categories.sql` — run it against a Supabase project's SQL editor. It's a breaking change to the old flat tournament schema (drops and recreates `tournament_courts`/`tournament_teams`/`tournament_matches`, adds the new tables above) — export anything worth keeping from an existing tournament first.
...
A category-based tournament engine: one tournament (name, sport, venue, dates) runs one or more **categories** in parallel — Men's Doubles, Mixed Doubles, a skill bracket, whatever the event needs — each with its own bracket format, entry fee, capacity and roster. Categories, not the tournament itself, are where almost everything happens: registration, fixtures, scoring, standings. Like sessions/shop, there's no login for players: category rosters, fixtures, and standings are public and real-time (Supabase Realtime on `tournament_categories`/`tournament_groups`/`tournament_teams`/`tournament_matches`). Category/group/team/match CRUD in `/admin` goes straight to Supabase with the organizer's authenticated session, the same `admin_all_<table>` RLS pattern as the rest of `/admin`; public registration, payment, session-sync, and bulk CSV import are real server-side actions with capacity/race-safety logic, so those go through `api/tournament.js` (action-dispatched behind one Serverless Function slot, same pattern as `api/shop.js`).
...
1. **New Tournament** — name, sport, venue, dates (all but name optional; add/edit later from the tournament's Details section). No session picker here — session linking is per category (step 3).
...
**Registration** (each category tab): a category's status controls what's visible/open — `setup` (organizer staging it, invisible on `/tournament`), `registration_open` (players can sign up), `registration_closed`, `active`, `completed`. Three ways teams get onto a category's roster, freely mixed:
- **Public self-registration** on `/tournament` (once a category is `registration_open`): a compact team form (player 1 + partner if doubles, phone, optional email/team name). Free categories register immediately; paid ones go through the same Razorpay hold-then-confirm flow as session registration (`create-order` → checkout → `confirm-payment`), or fall back to "pay by UPI, organizer marks it paid" if no Razorpay key is configured — exactly mirroring the session and shop checkout flows. A category with `max_teams` set waitlists new entries once full (confirmed teams only count against the cap; the organizer promotes off the waitlist from the Registrations tab).
...
- **Auto-sync from a linked session** — see below, same mechanism the original tournament engine used, now scoped per category instead of per tournament.
...
**Export for DUPR:** each category's Export tab downloads every one of *its* completed matches (group stage and knockout alike) as a CSV in DUPR's exact bulk-match-upload column format (`matchType, scoreType, event, date, playerA1, playerA1DuprId, ..., teamAGame1..teamBGame5`) — ready to upload as-is. `matchType` is `D`/`S` depending on whether a team has a second player; `event` folds in the tournament name, category name, and group/stage name, standing in for DUPR's "bracket"; only game 1 is ever filled in, since this engine scores a single game per match. Player DUPR IDs are pulled from `players.dupr_id`/`partner_dupr_id` for auto-synced teams (via `source_player_id`) and left blank for directly-registered or hand-added teams — the export flags how many matches are missing one.

**Auto-sync from a linked session** (Registrations tab → Linked Session): when a category has `session_id` set, its teams stay in sync with that session's registrations automatically — no admin action needed for registrations that happen *after* the link. A Postgres trigger (`sync_tournament_team_from_player`, fires on `players` insert/update) creates a team the moment a registration on that session is `confirmed` (and, for a doubles category, has a partner name set), placing it on whichever of the category's groups currently has the fewest teams. This runs server-side regardless of whether `/admin` is open, and regardless of *how* the `players` row changed — a new registration via `/events`, an admin promoting someone off the waitlist, or marking someone withdrawn all go through direct writes to `players` (the last two straight from the browser) — so this has to stay a DB trigger rather than move into application code; there's no HTTP request to hook for a browser-direct write. The trigger's per-player placement logic lives in a small SQL helper, `sync_team_for_player_in_category(category_id, player_id)`.

Registrations that already existed *before* a session gets linked aren't picked up by the trigger. That's what the **Sync teams from session** button is for — a single, explicit, admin-invoked action backed by `POST /api/tournament` (`action: sync-teams`), running the same qualification/placement rule (`api/_lib/tournamentSync.js`, pure and unit-tested) against every registration on the linked session and reporting how many new teams it created. Safe to click repeatedly — it only ever adds a team for a registration that doesn't have one yet in that category.

Neither sync path ever edits or deletes an existing team. If a player withdraws their session registration after their team already exists, the team isn't touched or removed (pulling a team after fixtures exist would corrupt the bracket) — `/admin` shows a "Withdrawn" badge next to that team everywhere it appears instead, so the organizer decides what to do. Independently, a directly-registered team can be withdrawn (soft, keeps match history intact) or removed outright from the Registrations tab at any time.
