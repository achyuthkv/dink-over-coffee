...
4. **Standings** (`computeStandings`, same lib) are derived live from completed matches — wins, then point differential, then points scored, the standard round-robin tiebreak order. There's no separate standings table to keep in sync; it's always a pure function of the match results. On the public page these are shown as one combined ranking across every court (with a Court column so viewers still know each team's pool) rather than a separate table per court — courts are round-robin pools, not divisions the site treats as separately ranked. Fixtures stay grouped per court underneath, since that's how the matches are actually played. Admin's per-court views (while scoring, and while picking who advances) still show standings court-by-court, since that's the operationally relevant grouping there.
